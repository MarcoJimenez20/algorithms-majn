...
public class Challenges implements IChallenges{
    ...
    public ResponseAlgorithms balancedParentheses(RequestString input) {
        ...
        Optional<String> optionalInput = Optional.of(input.getInputString());
        Boolean valid = balancedString(optionalInput);
        log.info("Es valido?: {}", valid.toString());
        Map<String, String> answer = new HashMap<>();
        answer.put("output", valid ? "valid": "invalid");
        answer.put("input", optionalInput.get());
        responseBalancedParentheses.setAnswer(answer);
        return responseBalancedParentheses;
    }

    public Boolean balancedString(Optional<String> optionalInput){
        int bandera = 0;
        if (!optionalInput.isPresent() || optionalInput.isEmpty()) {
            return true;
        }

        for (char c : optionalInput.get().toCharArray()) {
            if (c == '(') {
                bandera++;
            } else if (c == ')') {
                bandera--;
